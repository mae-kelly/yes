<!-- BusinessUnitMetrics.svelte - Enhanced with Perfect Screen Fit -->
<script>
	...
	$: threatDistribution = filteredBUs.reduce((acc, [_, count]) => {
...
<div class="bu-dashboard">
		...
		<div class="header-content">
			<div class="title-block">
				...
				<div class="title-text">
					<h1>BUSINESS UNIT MATRIX</h1>
					<p>Organizational Asset Distribution</p>
				</div>
			</div>
			<div class="metrics-row">
				...
				<div class="metric-card primary">
					<div class="metric-value">{filteredBUs[0] ? filteredBUs[0][0].substring(0, 15) : 'N/A'}</div>
					<div class="metric-label">TOP UNIT</div>
				</div>
	...
	<div class="main-content">
		...
		<div class="table-panel">
							...
							<tbody>
								{#each buDetails as host}
									<tr>
										...
										<td>
											<span class="bu-badge">ASSIGNED</span>
										</td>
									</tr>
								{/each}
							</tbody>
						...
						<tbody>
							{#each paginatedBUs as [bu, count]}
								<tr>
									...
									<td class="center">
										<button class="drill-btn" on:click={() => drillDownBU(bu, count)}>
											ANALYZE →
										</button>
									</td>
								</tr>
			...
			{:else}
				...
				<div class="grid-container">
					{#each paginatedBUs as [bu, count]}
						<div class="grid-card" style="--card-color: {getThreatLevel(count).color}" on:click={() => drillDownBU(bu, count)}>
							<div class="card-header">
								<span class="card-icon">🏢</span>
								<span class="threat-indicator {getThreatLevel(count).level.toLowerCase()}">{getThreatLevel(count).level}</span>
							</div>
							<div class="card-body">
								<div class="bu-name">{bu.substring(0, 20)}{bu.length > 20 ? '...' : ''}</div>
								<div class="bu-count">{count.toLocaleString()}</div>
		...
		<div class="viz-panel">
						...
						{#if Object.keys(categoryBreakdown).length > 0}
							{#each Object.entries(categoryBreakdown) as [category, count], i}
								<circle
									...
									r={60 - i * 8}
									fill="none"
									stroke={category === 'Technology' ? '#00ffff' : 
										category === 'Operations' ? '#ff00ff' : 
										category === 'Security' ? '#ff0066' :
										category === 'Finance' ? '#ffaa00' : '#0096ff'}
									stroke-width="15"
									stroke-dasharray={`${(count / Object.values(categoryBreakdown).reduce((a,b) => a+b, 0) * 377)} 377`}
			...
			<!-- Top Business Units -->
			<div class="viz-card">
				<h4>TOP 5 BUSINESS UNITS</h4>
				<div class="bar-chart">
					{#each filteredBUs.slice(0, 5) as [bu, count]}
						<div class="bar-item">
							<div class="bar-label">{bu.substring(0, 25)}{bu.length > 25 ? '...' : ''}</div>
			...
			<div class="viz-card">
				...
				<div class="threat-grid">
					{#each Object.entries(threatDistribution) as [level, count]}
						<div class="threat-cell" style="--threat-color: {level === 'CRITICAL' ? '#ff00ff' : level === 'HIGH' ? '#ff0066' : level === 'MEDIUM' ? '#ffaa00' : '#00ffff'}">
							<div class="threat-level">{level}</div>
							<div class="threat-count">{count}</div>
							<div class="threat-bar">
								<div class="threat-fill" style="width: {(count/filteredBUs.length)*100}%; background: var(--threat-color)"></div>
							</div>
						</div>
					{/each}
				</div>
			...
			<div class="viz-card">
				...
				<div class="matrix-grid">
					{#each paginatedBUs.slice(0, 9) as [bu, count]}
						<div class="matrix-cell" style="background: {getThreatLevel(count).color}20; border-color: {getThreatLevel(count).color}">
							...
							<div class="cell-label">{bu.substring(0, 8)}</div>
						</div>
					{/each}
				</div>
			</div>
