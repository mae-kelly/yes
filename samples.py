...
class ColumnSampleExtractor:
    ...
    def _extract_samples_for_type(self, json_file: Path, column_type: str):
        ...
        try:
            ...
            if not data or 'columns' not in data:
                logger.warning(f"No column data found in {json_file.name}")
                return
            
            # Collect all unique projects
            projects = set()
            for table_path in data['columns'].keys():
            ...
            all_samples = defaultdict(lambda: {
                'column_name': '',
                'table_path': '',
                'samples': [],
                'unique_values': set(),
            ...
            # Group columns by their actual column name
            columns_by_name = defaultdict(list)
            for table_path, column_name in data['columns'].items():
                columns_by_name[column_name].append(table_path)
            
            # Process each unique column name
    ...
    def _get_column_samples(self, column_name: str, table_paths: List[str]) -> Dict[str, Any]:
        ...
        # Randomly select tables to sample from
        tables_to_sample = random.sample(table_paths, min(max_tables_to_check, len(table_paths)))
            ...
            try:
                ...
                with manager.get_client() as client:
                    ...
                    try:
                        ...
                        for row in results[:samples_per_table]:
                            value = row.value
                        ...
                        # Also get null count for this table
                        null_query = f"""
                        ...
                        null_job = client.query(null_query)
                        null_result = list(null_job.result(timeout=30))
                        if null_result:
                            column_data['null_count'] += null_result[0].null_count
                        ...
                        total_result = list(total_job.result(timeout=30))
                        if total_result:
                            column_data['total_rows_checked'] += total_result[0].total
                        
                        logger.debug(f"       Collected {len(results)} samples from {table_path}")
                        
                    except Exception as e:
                        logger.debug(f"       Failed to query {table_path}: {e}")
                        continue
                    
            except Exception as e:
                logger.debug(f"Failed to get samples from {table_path}: {e}")
                continue
    ...
    def _save_samples(self, samples: Dict[str, Any], output_file: Path, column_type: str):
        ...
        output_data = {
            'metadata': {
                ...
                'total_columns_found': len(samples)
            },
            'columns': {}
        }
        ...
        for column_name, column_data in samples.items():
            output_data['columns'][column_name] = {
                ...
                'tables_found_in': column_data.get('tables', []),
                'total_tables': len(column_data.get('tables', [])),
                'samples': column_data['samples'][:self.sample_size],
                'sample_count': len(column_data['samples']),
                'unique_value_count': len(column_data['unique_values']),
                'unique_values_sample': column_data['unique_values'][:20],
                'null_count': column_data['null_count'],
                'total_rows_checked': column_data['total_rows_checked'],
                'null_percentage': (column_data['null_count'] / column_data['total_rows_checked'] * 100) 
                                  if column_data['total_rows_checked'] > 0 else 0,
                'top_10_values': list(column_data.get('value_distribution', {}).items())[:10],
    ...
    def _generate_summary_report(self, samples: Dict[str, Any], column_type: str):
        """Generate a summary report for the column type"""
        ...
        with open(report_file, 'w') as f:
            ...
            for column_name, column_data in samples.items():
                ...
                f.write("-" * 40 + "\n")
                f.write(f"Found in {len(column_data.get('tables', []))} tables\n")
                f.write(f"Samples collected: {len(column_data['samples'])}\n")
                f.write(f"Unique values: {len(column_data['unique_values'])}\n")
                f.write(f"Null percentage: {(column_data['null_count'] / max(column_data['total_rows_checked'], 1) * 100):.1f}%\n")
                
                if column_data['samples']:
