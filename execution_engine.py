...
class ExecutionEngine:
    ...
    def _aggressive_algorithm(self, request: Dict) -> Dict:
        try:
            ...
            print(f"📊 Placing {order_type} order: {side} {size} {symbol}")
            ...
            try:
                if order_type == 'market':
                    order_result = self.okx_client.create_market_order(
                        symbol, side, size, None, None, {'reduceOnly': reduce_only}
                    )
                else:
                    order_result = self.okx_client.create_limit_order(
                        symbol, side, size, price, {'reduceOnly': reduce_only}
                    )
                    
                execution_time = time.time() - start_time
                
                if order_result and order_result.get('id'):
                    print(f"✅ Order placed successfully: {order_result['id']}")
                    ...
                    if filled_order and filled_order.get('status') in ['closed', 'filled']:
                        ...
                        avg_price = float(filled_order.get('average', 0))
                        ...
                        self.execution_metrics['total_volume_executed'] += filled_size
                        self.execution_metrics['total_fees_paid'] += fees
                        
                        avg_time = self.execution_metrics['average_execution_time']
                        total_orders = self.execution_metrics['orders_filled']
                        self.execution_metrics['average_execution_time'] = (
                            (avg_time * (total_orders - 1) + execution_time) / total_orders
                        )
                        
                        print(f"✅ Order filled: {filled_size} @ ${avg_price:.4f}")
    ...
    def _twap_algorithm(self, request: Dict) -> Dict:
        try:
            ...
            print(f"📊 Starting TWAP execution: {side} {total_size} {symbol} over {duration_minutes}min")
            ...
            for i in range(slice_count):
                try:
                    print(f"📊 TWAP slice {i+1}/{slice_count}")
                    ...
                    slice_order = self.okx_client.create_limit_order(
                        symbol, side, slice_size, limit_price
                    )
                    
                    if slice_order and slice_order.get('id'):
                        filled_order = self._wait_for_fill(
                            slice_order['id'], symbol, timeout=min(slice_interval, 60)
                        )
                        ...
                        if filled_order and filled_order.get('status') in ['closed', 'filled']:
                            ...
                            total_filled += fill_data['filled_size']
                            print(f"✅ TWAP slice filled: {fill_data['filled_size']}")
                        else:
                            try:
                            ...
                            if market_order:
                                ...
                                if market_filled and market_filled.get('status') in ['closed', 'filled']:
                                    fill_data = {
                    ...
                    if i < slice_count - 1:
                        print(f"⏱️  Waiting {slice_interval:.1f}s for next slice...")
            ...
            if fills:
                ...
                print(f"✅ TWAP completed: {total_filled} @ ${average_price:.4f}")
    ...
    def _vwap_algorithm(self, request: Dict) -> Dict:
        try:
            ...
            print(f"📊 Starting VWAP execution: {side} {total_size} {symbol}")
            ...
            for i, period_volume_ratio in enumerate(historical_volumes):
                slice_size = total_size * period_volume_ratio
                
                if slice_size < 0.001:
                    continue
                    
                print(f"📊 VWAP slice {i+1}: {slice_size} ({period_volume_ratio:.1%} of volume)")
            ...
            if fills:
                ...
                print(f"✅ VWAP completed: {total_filled} @ ${average_price:.4f}")
    ...
    def _iceberg_algorithm(self, request: Dict) -> Dict:
        try:
            ...
            print(f"📊 Starting Iceberg execution: {side} {total_size} {symbol} (chunks: {iceberg_size})")
            ...
            while total_filled < total_size:
                ...
                print(f"📊 Iceberg slice: {current_slice}")
                ...
                slice_order = self.okx_client.create_limit_order(
                    symbol, side, current_slice, limit_price, {'timeInForce': 'GTC'}
                )
                ...
                if slice_order and slice_order.get('id'):
                    ...
                    if filled_order and filled_order.get('status') in ['closed', 'filled']:
                        ...
                        fills.append(fill_data)
                        total_filled += fill_data['filled_size']
                        print(f"✅ Iceberg slice filled: {fill_data['filled_size']}")
                    else:
                        try:
                            self.okx_client.cancel_order(slice_order['id'], symbol)
                        except:
            ...
            if fills:
                ...
                print(f"✅ Iceberg completed: {total_filled} @ ${average_price:.4f}")
    ...
    def _adaptive_algorithm(self, request: Dict) -> Dict:
        try:
            ...
            print(f"📊 Adaptive execution based on market conditions:")
            print(f"   Volatility: {market_conditions['volatility']:.3f}")
            print(f"   Liquidity: {market_conditions['liquidity']:.3f}")
            print(f"   Spread: {market_conditions['spread']:.4f}")
            
            if market_conditions['volatility'] > 0.05:
                if market_conditions['liquidity'] > 0.7:
                    print("📊 High vol + good liquidity → Aggressive")
                    return self._aggressive_algorithm(request)
                else:
                    print("📊 High vol + poor liquidity → TWAP")
                    request['duration'] = 45
                    request['slices'] = 15
                    return self._twap_algorithm(request)
            elif market_conditions['liquidity'] < 0.3:
                print("📊 Poor liquidity → Iceberg")
                request['iceberg_size'] = request['size'] * 0.05
                return self._iceberg_algorithm(request)
            else:
                print("📊 Normal conditions → Aggressive")
                return self._aggressive_algorithm(request)
    ...
    def _get_volume_profile(self, symbol: str, duration_minutes: int) -> List[float]:
        try:
            ...
            if not historical_data:
                return [1.0 / duration_minutes] * duration_minutes
